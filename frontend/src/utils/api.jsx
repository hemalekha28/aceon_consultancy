...
export const api = {
  ...
  uploadProductImages: async (files) => {
    try {
      ...
      console.error('API error uploading images:', err);
  ...
  bulkCreateProducts: async (products) => {
    try {
      ...
      console.error('API error bulk creating products:', err);
      throw new Error(err.response?.data?.message || err.message || 'Failed to bulk create products');
    }
  },
